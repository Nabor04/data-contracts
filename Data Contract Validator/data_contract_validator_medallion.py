...
# =============================================================================
# Architektur (korrekte Datenfluss-Richtung):
#
#   PRODUCER
#      ↓
#   BRONZE  ← Dateien werden MANUELL hier abgelegt
#      ↓
#   VALIDIERUNG gegen Data Contract
#      ↓              ↓
#   SILVER         QUARANTINE
#  (valide)       (invalide — direkt aus der Validierung, NICHT aus Silver!)
#      ↓
#   GOLD (aggregierte Kennzahlen aus Silver)
#
# Workflow:
#   1. CSV-Dateien manuell in  medallion/bronze/CSV/  hochladen
#   2. XML-Dateien manuell in  medallion/bronze/XML/  hochladen
#   3. Dieses Script ausführen — der Rest passiert automatisch
#
# =============================================================================
...
# =============================================================================
# Schritt 4: Silver, Quarantine und Gold vor jedem Lauf leeren
# =============================================================================

def ausgabe_schichten_leeren():
    """
    Löscht alle Dateien in Silver, Quarantine und Gold vor dem neuen Durchlauf.
    Bronze bleibt unberührt — dort liegen die Eingangsdateien.
    """
    # Gold wird NICHT geleert — dort liegt die inkrementelle History
    zu_leerende_ordner = [
        SILVER_CSV_PFAD,
        SILVER_XML_PFAD,
        QUARANTINE_CSV_PFAD,
    ...
    print('  Silver/CSV, Silver/XML, Quarantine/CSV, Quarantine/XML, Gold → leer')
    print('  Bronze bleibt unverändert')

ausgabe_schichten_leeren()


# =============================================================================
# Schritt 6: Bronze-Schicht — Inhalt prüfen
# =============================================================================

def bronze_schicht_pruefen():
    """
    Prüft ob Dateien in Bronze vorhanden sind.
    Bronze wird MANUELL befüllt — das Script liest nur daraus.
    Wirft einen Fehler, wenn Bronze komplett leer ist.
    """
    csv_dateien = []
    xml_dateien = []
...
# =============================================================================
# Schritt 5: Validierungslogik (CSV mit Great Expectations)
# =============================================================================
import pandas as pd
...
def format_expectation_details(expectation_kwargs):
    """Gibt relevante Erwartungsparameter als lesbaren String zurück."""
...
def validate_csv_und_schichten_befuellen(contract, bronze_pfad):
    """
    Validiert eine CSV-Datei aus der Bronze-Schicht.
    → Valide Zeilen  → Silver
    → Invalide Zeilen → Quarantine  (aus der VALIDIERUNG, nicht aus Silver!)
    Gibt die Verstöße zurück.
    """
    ...
    suite.add_expectation(gx.expectations.ExpectTableRowCountToBeBetween(
        min_value=rc.get('minimum', 1), max_value=rc.get('maximum', 1_000_000)
    ))
...
# =============================================================================
# Schritt 6: Validierungslogik (XML)
# =============================================================================
import re
...
def wert_parsen(rohwert, erwarteter_typ):
    ...
    if erwarteter_typ == 'integer':
        try: return int(float(bereinigt))
        except: return None
    return bereinigt
...
def validate_xml_und_schichten_befuellen(contract, bronze_pfad):
    """
    Validiert eine XML-Datei aus der Bronze-Schicht.
    → Keine Verstöße  → Silver
    → Verstöße        → Quarantine  (aus der VALIDIERUNG, nicht aus Silver!)
    """
    schema             = contract['dataContract']['schema']
...
# =============================================================================
# Schritt 8: Gold-Schicht — Kennzahlen aus Silver aggregieren
# =============================================================================

def gold_schicht_befuellen(anzahl_verstoesse, zeitpunkt):
    """
    Aggregiert Kennzahlen aus den validierten Silver-Daten.
    Schreibt einen neuen Eintrag inkrementell in medallion_summary.csv —
    bestehende Läufe bleiben erhalten.
    """
...
# =============================================================================
# Schritt 7: Bronze direkt verarbeiten (manuell befüllt)
# =============================================================================

def verarbeite_bronze_csv(contract):
    """Liest alle CSV-Dateien aus Bronze, verarbeitet sie mit Fehlerbehandlung."""
    ...
    for dateiname in dateien:
        ...
        try:
            ...
            # Datei trotzdem in Quarantine verschieben damit Bronze leer bleibt
            try:
...
def verarbeite_bronze_xml(contract):
    """Liest alle XML-Dateien aus Bronze, verarbeitet sie mit Fehlerbehandlung."""
...
# =============================================================================
# Schritt 9: Vollständiger Medallion-Durchlauf
# =============================================================================
...
# =============================================================================
# Schritt 10: HTML-Validierungsbericht
# =============================================================================
from jinja2 import Template

# Bericht in Gold ablegen (inkrementell nummeriert)
berichtsnummer = 1
