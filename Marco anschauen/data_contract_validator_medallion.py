...
def validate_csv_und_schichten_befuellen(contract, bronze_pfad):
    ...
    try:
        ...
        quarantine_ziel = os.path.join(QUARANTINE_CSV_PFAD, dateiname)
        shutil.move(bronze_pfad, quarantine_ziel)
        print(f'  → Quarantine/CSV: {dateiname}  (Lesefehler: {type(lesefehler).__name__})')
        return verstoesse
    ...
    if not verstoesse:
        # Datei vollständig valide → Silver (neu schreiben), Bronze löschen
        ...
        # Datei hat Fehler → Bronze direkt nach Quarantine verschieben (Name unverändert)
        quarantine_ziel = os.path.join(QUARANTINE_CSV_PFAD, dateiname)
        shutil.move(bronze_pfad, quarantine_ziel)
        print(f'  → Quarantine/CSV: {dateiname}  ({len(verstoesse)} Verstöße)')
...
def validate_xml_und_schichten_befuellen(contract, bronze_pfad):
    ...
    if verstoesse:
        ...
        shutil.move(bronze_pfad, os.path.join(QUARANTINE_XML_PFAD, dateiname))
        print(f'  Quarantine/XML ← {dateiname}  ({len(verstoesse)} Verstoß/Verstöße)')
    else:
